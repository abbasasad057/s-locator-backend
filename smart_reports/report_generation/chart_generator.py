...
def plot_healthcare_vs_competition(
    ...
    for site in top_n_sites:
        # X-axis: Sum of hospitals and dentists from healthcare data
        num_hospitals = site.get("num_of_hospitals", 0) or 0
        num_dentists = site.get("num_of_dentists", 0) or 0
        healthcare_total = num_hospitals + num_dentists
        healthcare_counts.append(healthcare_total)
        
        # Y-axis: Number of competing pharmacies
        num_pharmacies = site.get("num_of_pharmacies", 0) or 0
        competition_counts.append(num_pharmacies)
    ...
    # --- Titles and Labels ---
    ax.set_title(
        "Healthcare Count vs Competition",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
    ax.set_xlabel(
        "Number of Healthcare Facilities (Hospitals & Dentists)", fontsize=12
    )
    ...
    plt.close(fig)
    logging.info(
        f"Successfully generated healthcare vs competition chart at {outpath}"
    )
