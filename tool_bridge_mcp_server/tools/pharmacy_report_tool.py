...
import sys
import json
import aiohttp  # Restored for API calls
...
try:
    ...
    port = CONF.port if hasattr(CONF, 'port') else 8000
    # Use BACKEND_URL env var for Docker, fallback to localhost for local dev
    FASTAPI_BASE_URL = os.getenv("BACKEND_URL", f"http://localhost:{port}")
except ImportError:
    ...
    logger.warning("Could not import CONF, using default FASTAPI_BASE_URL")
        ...
        try:
            ...
            if "reports" in html_file_path:
                ...
                if len(parts) > 1:
                    ...
                    # Use full backend URL (port 8000) for report access
                    web_url = f"{FASTAPI_BASE_URL}/static/reports/{relative_path}"
                else:
            ...
            logger.info(f"Using backend URL: {FASTAPI_BASE_URL}")
...
"""
    ...
    #     """Generate a simple markdown report with click here message (LEGACY - hardcoded path)"""
    #     try:
    #         current_date = datetime.now().strftime("%B %d, %Y")
    #
